...
use feff_core::modules::fms::FmsModule;
...
const REQUIRED_FMS_INPUT_ARTIFACTS: [&str; 3] = ["fms.inp", "geom.dat", "global.inp"];
...
#[test]
fn approved_fms_fixtures_emit_required_true_compute_artifacts() {
    for fixture in &APPROVED_FMS_FIXTURES {
        let temp = TempDir::new().expect("tempdir should be created");
        let output_dir = run_rdinp_pot_xsph_and_fms_for_fixture(fixture, temp.path(), "actual");

        for artifact in &EXPECTED_FMS_ARTIFACTS {
            let output_path = output_dir.join(artifact);
            assert!(
                output_path.is_file(),
                "FMS artifact '{}' should exist for fixture '{}'",
                output_path.display(),
                fixture.id
            );
            assert!(
                !fs::read(&output_path)
                    ...
                    .is_empty(),
                "FMS artifact '{}' should not be empty",
                output_path.display()
            );
        }
    }
...
#[test]
fn approved_fms_fixtures_are_deterministic_across_runs() {
    for fixture in &APPROVED_FMS_FIXTURES {
        let temp = TempDir::new().expect("tempdir should be created");
        let first_output = run_rdinp_pot_xsph_and_fms_for_fixture(fixture, temp.path(), "first");
        let second_output = run_rdinp_pot_xsph_and_fms_for_fixture(fixture, temp.path(), "second");
...
fn fms_regression_suite_passes() {
    ...
    for fixture in &APPROVED_FMS_FIXTURES {
        ...
        let seed_output = run_rdinp_pot_xsph_and_fms_for_fixture(fixture, &seed_root, "actual");
    ...
    let manifest = json!({
      "fixtures": APPROVED_FMS_FIXTURES.iter().map(|fixture| {
        json!({
          "id": fixture.id,
          "modulesCovered": ["RDINP", "POT", "XSPH", "FMS"],
...
fn run_rdinp_pot_xsph_and_fms_for_fixture(
    fixture: &FixtureCase,
    root: &Path,
    subdir: &str,
) -> PathBuf {
    ...
    for artifact in REQUIRED_FMS_INPUT_ARTIFACTS {
        assert!(
            rdinp_set.contains(artifact),
            "fixture '{}' should include '{}' before POT/XSPH/FMS execution",
            fixture.id,
            artifact
        );
    }
    ...
    assert_eq!(
        ...
        "fixture '{}' should emit expected XSPH artifacts before FMS execution",
        fixture.id
    );
    ...
    assert_eq!(
        artifact_set(&fms_artifacts),
        expected_artifact_set(&EXPECTED_FMS_ARTIFACTS),
        "fixture '{}' should emit expected FMS artifacts",
        fixture.id
    );
