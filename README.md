...
Use `--run-xsph` to execute the Rust XSPH scaffold path before comparisons; it expects staged `xsph.inp`, `geom.dat`, `global.inp`, and `pot.bin` (optionally `wscrn.dat`) in each fixture actual output directory and writes deterministic scaffold artifacts (`phase.bin`, `xsect.dat`, `log2.dat`).
