...
Runtime compute engines are currently available for `RDINP`, `POT`, `SCREEN`, `CRPA`, `XSPH`, `PATH`, and `FMS`; commands for modules that have not been ported to true-compute yet return `RUN.RUNTIME_ENGINE_UNAVAILABLE` with exit code `4`.
