...
Runtime compute engines are currently available for `RDINP`, `POT`, `SCREEN`, `CRPA`, `XSPH`, `PATH`, `FMS`, `BAND`, `LDOS`, `COMPTON`, and `DEBYE`; commands for modules that have not been ported to true-compute yet return `RUN.RUNTIME_ENGINE_UNAVAILABLE` with exit code `4`.
