...
Use `--run-fms` to execute the Rust FMS scaffold before comparisons; it expects staged `fms.inp`, `geom.dat`, `global.inp`, and `phase.bin` in each fixture actual output directory and materializes deterministic scaffold placeholders (`gg.bin`, `log3.dat`).
